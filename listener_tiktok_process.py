...
from listener import listen_caption_facebook, listen_caption_tiktok
...
userId_list = pd.read_csv(f'tiktok_fans/user_id.csv', encoding="utf-8")
...
def run(op_i, platformType_run, run_maxProcesses):
    ...
    numberCycles = math.ceil(len(browser_id_set) / run_maxProcesses)

    cycle_count = 1
    for count_i in range(numberCycles):
        # 随机选取N个浏览器 N = numberOfProcesses
        try:
            current_browser_id_list = random.sample(browser_id_set, run_maxProcesses)
        except ValueError:
...
if __name__ == "__main__":
    ...
    # 最大进程数
    maxProcesses = bro_length
    temp_add = 0

    run(0, platformType, bro_length, 0)
    # while temp_add < len(group_list):
    #     run(0, loop_platformType, bro_length, temp_add)
