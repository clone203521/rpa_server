...
account_list = pd.read_csv('utils/fb_account2.csv', encoding='utf8')
...
@reset_file
def run2(op_i, platformType_run, maxProcesses):
    model_list_run2 = ['login_init', 'brushPost', 'joinGroup']
...
if __name__ == "__main__":
    ...
    platformType = 'facebook_init'
    op_index = 0
    # for operate_index in range(1):
    #     run2(op_index_list[operate_index], loop_platformType)

    run2(0, platformType_run=platformType, maxProcesses=10)
    # while True:
    #     run2(2, loop_platformType)
