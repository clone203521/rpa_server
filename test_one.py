...
def operate_tiktok(browser_id_op, model, temp_index, add_index, op_platformType):
    ...
    if val_flag:
        if model == 'modify_personal_data':
            ...
            if flag:
                os.remove(f'./browserDownload/{browser_id_op}/video_1.mp4')
...
def run(op_i, platformType_run, maxProcesses):
    ...
    #         pool.apply_async(operate_tiktok, args=(browser, model_list[operate_index_run],cycle_count, complete_browser_length,
...
@reset_file
def run_loop(start_index, loop_platformType, reset_video=False):
    i = start_index
    while True:
        if i < 2:
...
def test_open():
    ads_id = "jfdfetm"
    open_url = "http://http://127.0.0.1:50325/api/v1/browser/start?user_id=" + ads_id
...
def addTo_group(page_addGroup: Union[ChromiumPage, ChromiumTab]):
    if page_addGroup.url != 'https://web.whatsapp.com/':
        page_addGroup.get('https://web.whatsapp.com/')
        pass
