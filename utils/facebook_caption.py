...
# 刷短视频
def brushReel(page_brushReel, brush_user_id):
...
# 加入指定小组
def joinAGroup(page_joinAGroup, video_user_id, group_url):
...
def brushVideo(page_brushVideo, video_user_id):
...
def brushPost(page_brushPost, post_user_id):
...
def face_init(page_init, email_init, pwd_init, fa_two_init, user_id_init):
...
if __name__ == '__main__':
    # 创建一个TOTP对象
    totp = pyotp.TOTP('KFRBCJQB2JVVH7AJGALUSUEMYC7BIEXV')
    # 生成当前时间的验证码
    print(totp.now())
